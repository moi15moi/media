...
Java_androidx_media3_decoder_ass_AssLibrary_initAssLibrary(JNIEnv *env, jclass thiz) {
...
Java_androidx_media3_decoder_ass_AssLibrary_destroyAssLibrary(JNIEnv *env, jclass clazz, jlong ass_library_ptr) {
...
extern "C"
JNIEXPORT void JNICALL
Java_androidx_media3_decoder_ass_AssRenderer_addFont(
    JNIEnv *env,
    jobject thiz,
  ...
  const char *name = env->GetStringUTFChars(font_name, 0);
  if (!name) {
  ...
  jbyte *data = env->GetByteArrayElements(font_data, 0);
  if (!data) {
